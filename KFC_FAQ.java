...
public class KFC_FAQ {
    ...
    public static void printFAQ() {
        String FAQFile = "./resources/Shop'sFAQ.txt", response; // file location        
        String CustomerQuestionFile = "./resources/CustomerQuestion.txt"; // file location
        Scanner scan = new Scanner(System.in);
        KFC_FAQ.aLoginMessage(); // prompt with the header message
        File_IO.printFile(FAQFile); // print the official FAQ file to the user 

        System.out.println("\n--------------------------------------------------------------------------------");
        System.out.println("[DID YOU GET THE ANSWER YOU WERE LOOKING FOR?\tY  |  N]");
        while (true) {
            response = scan.nextLine(); // take user response
            if (response.trim().equalsIgnoreCase("Y")) {
                System.out.println("[WE ARE HAPPY WE WERE ABLE TO ANSWER YOUR QUESTION, HAVE A GOO DAY!]\n"); // prompt with message
                break;
            } else if (response.trim().equalsIgnoreCase("N")) {
                recordQuestion(scan, CustomerQuestionFile); // record the question if not statisfied with the current answer
                break;
            } else {
                System.out.println("\n[INVALID INPUT\tPLEASE ENTER\tY  |  N]"); // error message
            }
        }
